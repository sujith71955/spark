  ...
  override protected[sql] def doExecuteBroadcast[T](): broadcast.Broadcast[T] = {
    try {
      ...
      case ex: TimeoutException =>
        ...
        val appStatsListener = sqlContext.sparkSession.sharedState.statusStore
          .execution(sparkContext.
            getLocalProperty(SQLExecution.EXECUTION_ID_KEY).toInt).getOrElse(ex)
          .asInstanceOf[SQLExecutionUIData]
        appStatsListener.jobs.keySet.foreach { jobId =>
